...
def list_all_vehicles(vehicles):
    if vehicles == []:  # alternative: if not vehicles
        ...
        for index, vehicle in enumerate(vehicles):
            print ("%s) %s %s with %s km driven so far. Last service date: %s" % (index+1, vehicle.brand, vehicle.model, vehicle.km_done, vehicle.service_date))
...
def add_new_vehicle(vehicles):
    ...
    km_done_str = input("Please enter the amount of kilometers that vehicle has done so far (just a number): ")
...
def choose_vehicle(vehicles):
    ...
    print ("")
    selection = input("What vehicle number wold you like to choose? ")
...
def add_new_kilometers(vehicles):
    ...
    print( "")
    new_km_str = input("How many kilometers would you like to add to the existing ones? (enter only a number) ")
    print( "")
...
def change_service_date(vehicles):
    ...
    print( "")
    new_service_date = input("What is the new general service date for this vehicle? (DD.MM.YYYY) ")
    print( "")
...
def main():
    ...
    while True:
        ...
        choice = input("Which option would you like to choose? (a, b, c, d, e) ")
        print ""
